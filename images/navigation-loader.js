...
displayNavigationFromPage = () => {
    navigationPageText.then(data => {
        ...
        // Only expand the first two .toc--part items, hide the rest
        const tocParts = document.querySelectorAll(".toc--part");
        tocParts.forEach((nav, index) => {
            if (index >= 2) { // Hide items from index 2 onwards (3rd, 4th, etc.)
                if (!nav.classList.contains("toc--part_hidden"))
                    nav.classList.add("toc--part_hidden")
            } else { // Keep first two items expanded
                if (nav.classList.contains("toc--part_hidden"))
                    nav.classList.remove("toc--part_hidden")
            }
        })
    }).then(() => {
